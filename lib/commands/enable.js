...
async function run() {
  ...
  // Write hook scripts: try PATH, fall back to enable-time known path
  const hooks = {
    'pre-commit': `#!/bin/sh
# quality-gate pre-commit hook
QG=$(command -v quality-gate 2>/dev/null || true)
if [ -n "$QG" ] && [ -x "$QG" ]; then exec "$QG" pre-commit; fi
if [ -x "${CLI_PATH}" ]; then exec node "${CLI_PATH}" pre-commit; fi
...
QG=$(command -v quality-gate 2>/dev/null || true)
if [ -n "$QG" ] && [ -x "$QG" ]; then exec "$QG" pre-push; fi
if [ -x "${CLI_PATH}" ]; then exec node "${CLI_PATH}" pre-push; fi
echo "quality-gate: not found. Run: npm install -g github:hpuhsp/quality-gate"
exit 1
`
  }
...
`
  ...
  console.log('')
  console.log('   Gates: secret-scan · syntax-check · sql-injection · auto-format')
  console.log('   pre-push: test runner + coverage threshold')
}
