...
`)
  ...
  if (apiKey) {
    config.push('# NOTE: Store API keys in environment, not config files')
    config.push('# Run: export ANTHROPIC_API_KEY=sk-ant-...')
  }
  if (remoteRepo) {
  ...
  if (apiKey) {
    ...
    console.log(`\n⚠️  API key set for this session only.`)
    console.log(`   To persist, add to your shell profile:`)
    console.log(`   echo 'export ANTHROPIC_API_KEY=sk-ant-...' >> ~/.zshrc`)
  }
